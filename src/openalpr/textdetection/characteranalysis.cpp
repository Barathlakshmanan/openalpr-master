...
namespace alpr
{
  ...
  //displayImage(config, "thresholded image", pipeline_data->thresholds[i]);
    }
    ...
    PlateMask plateMask1(pipeline_data);
    plateMask1.findOuterBoxMask(allTextContours);
	
//std::cout<<std::endl<<"has plate mask? "<<plateMask1.hasPlateMask<<std::endl;

    pipeline_data->hasPlateBorder = plateMask1.hasPlateMask;
    pipeline_data->plateBorderMask = plateMask1.getMask();

    if (plateMask1.hasPlateMask)
    {
    ...
    LineFinder lf1(pipeline_data);
    vector<vector<Point> > linePolygons1 = lf1.findLines(pipeline_data->crop_gray, bestContours);

    vector<TextLine> tempTextLines1;
    for (unsigned int i = 0; i < linePolygons1.size(); i++)
    {
      vector<Point> linePolygon = linePolygons1[i];
      ...
      tempTextLines1.push_back(textLine);
    }

    filterBetweenLines(bestThreshold, bestContours, tempTextLines1);

    // Sort the lines from top to bottom.
    std::sort(tempTextLines1.begin(), tempTextLines1.end(), sort_text_line);

    // Now that we've filtered a few more contours, re-do the text area.
    for (unsigned int i = 0; i < tempTextLines1.size(); i++)
    {
      vector<Point> updatedTextArea = getCharArea(tempTextLines1[i].topLine, tempTextLines1[i].bottomLine);
      vector<Point> linePolygon = tempTextLines1[i].linePolygon;
...
//std::cout<<" result:"<<pipeline_data->plate_inverted<<std::endl;
...
//imshow("inverted imagee",pipeline_data->crop_gray);
...
allTextContours.clear();
    pipeline_data->textLines.clear();
pipeline_data->plate_corners.clear();
...
//cout<<allTextContours.size()<<"allTextContours.sixe()"<<endl;
    cout<<pipeline_data->thresholds.size();
    for (unsigned int i = 0; i < pipeline_data->thresholds.size(); i++)
    {
cout<<endl<<endl<<i;
      this->filter(pipeline_data->thresholds[i], allTextContours[i]);
...
//std::cout<<std::endl<<"has plate mask? "<<plateMask.hasPlateMask<<std::endl;
...
//std::cout<<std::endl<<std::endl<<NUM_STEPS<<std::endl<<"good indices: "<<goodIndices<<std::endl<<"segmentcount: "<<segmentCount<<std::endl<<std::endl<<std::endl;
      if (segmentCount > bestFitScore)
      {
        bestFitScore = segmentCount;
  ...
  void CharacterAnalysis::filterBetweenLines(Mat img, TextContours& textContours, vector<TextLine> textLines )
  {
    ...
    for (unsigned int i = 0; i < textContours.size(); i++)
    {
      ...
      if (percentInsideMask < MIN_AREA_PERCENT_WITHIN_LINES)
      {
        ...
        if (config->debugCharAnalysis)
          cout << "Rejecting this contour due to insufficient area, it dint fit between lines" << endl;
      ...
      for (unsigned int i = 0; i < textLines.size(); i++)
      {
        ...
        else
        {
          ...
          textContours.goodIndices[i] = false; 
          if (config->debugCharAnalysis)
            cout << "Rejecting due to top/bottom points that are out of range, distance is too much from top bottom line" << endl;
        }
      }
...
//imshow("check",tempMaskedContour);
//waitKey();
 ...
 //displayImage(config, "mask image", charMask);
	float areaBox=charMask.rows*charMask.cols;
float ratio=countNonZero(charMask)/areaBox;
//cout<<areaBox<<" ratio: "<<ratio<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl<<endl;
    Scalar meanVal = mean(bestThreshold, charMask)[0];

    if (this->config->debugCharAnalysis)
      cout << "CharacterAnalysis, plate inverted: MEAN: " << meanVal << " : " << bestThreshold.type() << "mean: "<<meanVal[0]<<endl;

    if ((meanVal[0] < 100)||(ratio<0.08))		// Half would be 122.5.  Give it a little extra oomf before saying it needs inversion.  Most states aren't inverted.
      return true;
  //     if (meanVal[0] >254)		// Half would be 122.5.  Give it a little extra oomf before saying it needs inversion.  Most states aren't inverted.
   //   return true;
  ...
  vector<Point> CharacterAnalysis::getCharArea(LineSegment topLine, LineSegment bottomLine)
  {
    ...
    {
//std::cout<<std::endl<<"New points obtained"<<std::endl;
      Point tl(leftX, topLine.getPointAt(leftX));
//std::cout<<leftX<<","<<topLine.getPointAt(leftX)<<std::endl;
      Point tr(rightX, topLine.getPointAt(rightX));
//std::cout<<rightX<<","<<topLine.getPointAt(rightX)<<std::endl;
      Point br(rightX, bottomLine.getPointAt(rightX));
//std::cout<<rightX<<","<<bottomLine.getPointAt(rightX)<<std::endl;

      Point bl(leftX, bottomLine.getPointAt(leftX));
//std::cout<<leftX<<","<<bottomLine.getPointAt(leftX)<<std::endl;
