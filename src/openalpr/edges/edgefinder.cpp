...
namespace alpr
{
  ...
  std::vector<cv::Point2f> EdgeFinder::findEdgeCorners() {
    ...
    if (tlc.longerSegment.length > tlc.charHeight * 3)
    {
      ...
      float verticalOffset = (idealPixelHeight * 1.75 / 2);
      float horizontalOffset = (idealPixelWidth * 1.5 / 2);
	...
	//std::cout<<"here are the points "<<topLeft<<","<<topRight<<","<<botLeft<<","<<botRight;
