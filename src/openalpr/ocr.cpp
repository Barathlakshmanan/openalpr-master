...
namespace alpr
{
  ...
  void OCR::performOCR(PipelineData* pipeline_data)
  {
    ...
    if (pipeline_data->charRegions.size() < config->postProcessMinCharacters){
      return;
std::cout<<"couldnt get sufficient characters";
}
std::cout<<endl<<"performing OCR"<<endl;
...
//if(j==0){ tesseract.Init(config->getTessdataPrefix().c_str(), "lch"	);
//   tesseract.SetVariable("save_blob_choices", "T");
//    tesseract.SetPageSegMode(PSM_SINGLE_CHAR);}
//else{ tesseract.Init(config->getTessdataPrefix().c_str(), "lch"	);
//   tesseract.SetVariable("save_blob_choices", "T");
 //   tesseract.SetPageSegMode(PSM_SINGLE_CHAR);}
