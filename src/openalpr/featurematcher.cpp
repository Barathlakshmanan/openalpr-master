...
namespace alpr
{
  ...
  FeatureMatcher::FeatureMatcher(Config* config)
  {
   ...
   this->descriptorMatcher = makePtr<BFMatcher>(int(NORM_HAMMING), false);

    //this->descriptorMatcher = DescriptorMatcher::create( "FlannBased" );

    //this->detector = makePtr<FastFeatureDetector>(10, true);
    this->detector = FastFeatureDetector::create(10, true);
    //this->extractor = makePtr<BRISK>(10, 1, 0.9);
                                             ...
                                             )
  {
    ...
    if (queryKeypoints.size() <= 5)
    {
      ...
      if (drawOnImage)
      {
        drawKeypoints(  queryImg, queryKeypoints, *outputImage, Scalar(0, 255, 0), DrawMatchesFlags::DEFAULT );
      }
      return result;
    }
    ...
    if (score > 0)
    {
      ...
      if (drawOnImage)
      {
        ...
        Mat tmpImg;
        drawKeypoints(  queryImg, queryKeypoints, tmpImg, Scalar(185, 0, 0), DrawMatchesFlags::DEFAULT );
        drawKeypoints(  tmpImg, positiveMatches, *outputImage, Scalar(0, 255, 0), DrawMatchesFlags::DEFAULT );
